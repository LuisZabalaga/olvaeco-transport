...
import { StylesEmployees } from './StylesEmployees';
...
const Employees = () => {
  ...
  const columns = [
   ...
   {
      name: 'Estado',
      selector: row => <div style={{backgroundColor: row.estadoEmpleado === 'ACTIVO' ? '#00FF41' : '#F71920', padding:'3px', borderRadius:'4px',color:'white'}}>{row.estadoEmpleado}</div>,
      sortable: true,
      width: '8%',
      wrap: true,
      reorder: true,
   ...
   const handleChangeHiring = (newValue: any) => {
      setHiringDate(format(new Date(newValue), "yyyy-MM-dd"));
      setEmployeeData({
        ...employeeData,
        "fechaContratacion": format(new Date(newValue), "yyyy-MM-dd")
      })
   };
   ...
   const handleGetAllEmployees = async() => {
      setLoading(true);
      await APISERVICE.getAllEmployees().then((res) => {
         console.log("LISTA DE EMPLEADOS", res);
   ...
   const handleGetEmployeeById = async(employeeId:any) => {
      setLoading(true);
      await APISERVICE.getEmployeeById(employeeId).then((res) => {
         console.log("EMPLEADO__ ", res);
         setEmployeeProperties(res);
         setLoading(false);
      })
      .catch((err) => {
         console.log("Error", err);
   ...
   const handleCreateEmployee = async() => {
      setLoading(true);
      await APISERVICE.createEmployeeUser(employeeData).then((res) => {
   ...
   const handleUpdatedEmployeeById = async() => {
      setLoading(true);
      await APISERVICE.updatedEmployeeById(employeeData, employeeId).then((res) => {
   ...
   const handleDeleteEmployeeById = async() => {
      setLoading(true);
      await APISERVICE.deleteEmployeeById(employeeId).then((res) => {
   ...
   const handleEditModal =  (employee: any) => {
      ...
      employeeData.nombre = employee.nombre;
      employeeData.apellidos = employee.apellidos;
      employeeData.dni = employee.dni;
      employeeData.email = employee.email;
      employeeData.telefono = employee.telefono;
      employeeData.fechaNacimiento = employee.fechaNacimiento;
      employeeData.fechaContratacion = employee.fechaContratacion;
      employeeData.estadoEmpleado = employee.estadoEmpleado;

      setBirthDate(employee.fechaNacimiento);
      setHiringDate(employee.fechaContratacion);
  ...
  console.log("data cargada", employeeProperties);
    ...
    <div>
      ...
      <Div sx={{bgcolor:'background.paper'}} className={styles.DivBody}>
          ...
          <DataTable
            ...
            pagination={listEmployees.length > 10 ? true : false}
            highlightOnHover
            pointerOnHover
          ...
          >            
            <DialogTitle id="alert-dialog-title" style={{fontSize:'18px', fontWeight:'500', textAlign:'center',margin:'0px'}}>
               Empleado:&nbsp; 
               {loading ? 'Sin Nombre' : employeeProperties["nombre"] + " " + employeeProperties["apellidos"]}
            </DialogTitle>
                  ...
                  loading ? (
                     ...
                     <List
                        ...
                        <ListSubheader component="div" id="nested-list-subheader">
                           ...
                           {employeeProperties ? employeeProperties["nombre"] : 'Sin Nombre'}
                        </ListSubheader>
                        }
                     >
                        <ListItemButton>
                           ...
                           <ListItemText primary="Nombres:" secondary={employeeProperties["nombre"]} />
                        </ListItemButton>
                        <ListItemButton>
                           <ListItemIcon>
                              <BsPersonSquare style={{color:'#024AC2',fontSize:'25px'}} />
                           </ListItemIcon>
                           <ListItemText primary="Apellidos:" secondary={employeeProperties["apellidos"]} />
                        </ListItemButton>
                        ...
                        <ListItemButton>
                           ...
                           <ListItemText primary="Email:"  secondary={employeeProperties["email"]}/>
                        </ListItemButton>
                        <Divider variant="inset" component="li" />
                        <ListItemButton>
                           <ListItemIcon>
                           ...
                           <ListItemText primary="Estado:" secondary={employeeProperties["estadoEmpleado"]} />
                        </ListItemButton>
                        {/* <Divider variant="inset" component="li" />
                        <ListItemButton>
                           <ListItemIcon>
                           ...
                           <ListItemText primary="Locales Asignados:" secondary={employeeProperties["establishment"] === '1' ? 'Farmacia 1' : 'Farmacia 2'} />
                        </ListItemButton>                */}
                     </List>
                  )
               }   
          ...
          {/* SHOW ADD OR EDIT MODAL EMPLEOYEE*/}
          <Dialog
              open={showEditModal}
              fullWidth={true}
              maxWidth="md"
          ...
          >
            ...
            <DialogActions>
               ...
               <LoadingButton
                  ...
                  onClick={employeeId ? handleUpdatedEmployeeById : handleCreateEmployee}
          ...
          >
              <DialogTitle id="alert-dialog-title" style={{fontSize:'18px', fontWeight:'500', textAlign:'center',margin:'0px'}}>
                  Empleado:&nbsp; 
                  {loading ? 'Sin Nombre' : employeeProperties["nombre"]}
              </DialogTitle>
              <DialogContent>
                {/* <DataTable
                  //   title='Organización'
